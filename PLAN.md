...
- ~~Pipelines tab → show logs~~ **shipped 2026-07-04**: build timeline + step logs in
  the tab (failed step auto-opens) and the `get_build_logs` copilot read tool.
- **CI-feedback loop** for the coding agent (propose → PR → CI red → agent reads log →
  pushes fix → green → human merges); log primitive shipped — remaining piece is
  push-to-own-branch (follow-up commits on the same `copilot/…` PR branch).
